...
def stitch_shapefile(group):
   for index, row in group.iterrows():
        assert 1 ==0, "grrrroup: {}".format(row["image_tile"])

#def stitch_shapefile(group: List[Dict[str, Any]]) -> Dict[str, Any]:
#    return group[0]

#def stitch_shapefile(group):
#  return group

def stitch_shapefile_df(group: pd.DataFrame):
...
def stitch_shapefile2(group: List[Dict[str, Any]]) -> Dict[str, Any]:
    image_shapefile_results = []
    temp_polygon_dict = defaultdict(dict)
    dict_ij = defaultdict(dict)
    image_data_from_arbitrary_row = group[0]["image_metadata"]
    for row in group:
        image_shapefile_results.extend(row["tile_shapefile_results"])
        image_tile = row["image_tile"]
        tile_num = image_tile.tile_metadata.tile_num
        temp_polygon_dict[tile_num] = len(row["tile_shapefile_results"])
        id_i = image_tile.tile_metadata.id_i
        id_j = image_tile.tile_metadata.id_j
        dict_ij[id_i][id_j] = tile_num
    ...
    group[0]["image_shapefile_results"] = image_shapefile_results_without_dups
    return group[0]
